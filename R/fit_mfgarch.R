...
fit_mfgarch <- function(df, y, x, K, low_freq = "Date", var_ratio_freq = NULL) {
  ...
  if ("Date" %in% colnames(df) == FALSE) {
    ...
    df_llh <- df %>% select_(., ~get(y), ~get(x), ~get(low_freq))
    ...
    if (K == 1) {
        lf <- function(parameters) {
            sum(likelihood_mg_mf(df = df_llh, y = y, x = x, low_freq = low_freq, mu = parameters["mu"], omega = 1 - parameters["alpha"] - parameters["beta"] - parameters["gamma"]/2, alpha = parameters["alpha"],
    ...
    if (K > 1) {
        lf <- function(parameters) {
            sum(likelihood_mg_mf(df = df_llh, y = y, x = x, low_freq = low_freq, mu = parameters["mu"], omega = 1 - parameters["alpha"] - parameters["beta"] - parameters["gamma"]/2,
        ...
        tau_forecast <-
          exp(sum_tau(m = p.e.nlminb$par["m"],
                  ...
                  covariate = c(df %>% select_(., ~get(x), ~get(low_freq)) %>%
    ...
    if (K > 1) {
        lf_mgarch_score <- function(parameters) {
            likelihood_mg_mf(df = df_llh, y = y, x = x, low_freq = low_freq, mu = parameters["mu"],
                             omega = 1 - parameters["alpha"] - parameters["beta"] - parameters["gamma"]/2,
                             alpha = parameters["alpha"],
                             beta = parameters["beta"],
                             gamma = parameters["gamma"],
