...
fit_mfgarch <- function(df, y, x, K, low.freq = "Date", var.ratio.freq = NULL) {
  ...
  if (K == 0) {
    lf_mgarch_score <- function(parameters) {
      likelihood_mg_simple(df = df.llh,
                           y = y,
                           mu = parameters["mu"],
                           omega = 1 - parameters["alpha"] - parameters["beta"] - parameters["gamma"]/2,
                           alpha = parameters["alpha"],
  ...
  GGsum <- t(G) %*% G
  #
  # p.e.nlminb.robust.standard.errors <-
  #   sqrt(diag(solve(GGsum))) * mean((df.fitted$residuals^2 - 1)^2, na.rm = TRUE)/2

  p.e.nlminb.robust.standard.errors <-
    (p.e.nlminb.n.i.hess %*% GGsum %*% p.e.nlminb.n.i.hess) %>%
    diag() %>%
    sqrt()
  ...
  if (K == 1) {
    return(list(mgarch = p.e.nlminb,
                broom.mgarch = data_frame(term = c("mu", "alpha", "beta", "gamma", "m", "theta", "llh"),
                                          estimate = c(p.e.nlminb$par[1:6], -p.e.nlminb$value),
  ...
  if (K > 1) {
    return(list(mgarch = p.e.nlminb,
                broom.mgarch = data_frame(term = c("mu", "alpha", "beta", "gamma", "m", "theta", "w2", "llh"),
                                          estimate = c(p.e.nlminb$par[1:7], -p.e.nlminb$value),
...
likelihood_mg_mf <- function(df, x, y, low.freq, mu, omega, alpha, beta, gamma, m, theta, w1 = 1, w2 = 1, g_zero, K = 2) {
  ...
  g <- calculate_g(omega = (1-alpha-beta),
                   alpha = alpha,
                   beta = beta,
                   gamma = gamma,
                   returns = ((ret-mu)/sqrt(tau)),
                   g0 = g_zero)
  if(sum(g <= 0) > 0) {
